...
class Query(graphene.ObjectType):
    ...
    # hikers_who_have_hiked_trail = graphene.List(HikeType, trailID=graphene.Int())  # possible with postgres
    hikers_who_have_hiked_trail = graphene.List(HikerType, trailID=graphene.Int())
    ...
    def resolve_hiker(self, info, hikerID):
        ...
        # return Hiker.objects.none().first()

    def resolve_trails(self, info):
        return Trail.objects.all() 

    def resolve_hikers_who_have_hiked_trail(self, info, trailID):
        trail = Trail.objects.get(id=trailID)
        # return Hike.objects.filter(trail=trail).order_by('hiker').distinct('hiker')  # possible with postgres
        hikerIDs = Hike.objects.filter(trail=trail).values('hiker').distinct()
        hikers = []
        for hiker in hikerIDs:
            hikers.append(Hiker.objects.get(id=hiker['hiker']))
        return hikers
